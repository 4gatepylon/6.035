...
public class CLI {
	...
	/**
	 * DEFAULT: produce default output. SCAN: scan the input and stop. PARSE: scan
	 * and parse input, and stop. INTER: produce a high-level intermediate
	 ...
	 * element is true, it indicates that the optimization named in the optnames[]
	 * parameter to parse with the same index should be performed.
	 */
	...
	/**
	 * Name of the file to get input from. This is null if the user didn't provide a
	 * file name.
	 */
	...
	/**
	 * Sets up default values for all of the result fields. Specifically, sets the
	 * input and output files to null, the target to DEFAULT, and the extra array to
	 * a new empty Vector.
	 */
	static {
		outfile = null;
	...
	/**
	 * Parse the command-line arguments. Sets all of the result fields accordingly.
	 * <BR>
	 *
	 * <TT>-t / --target= <I>target</I></TT> sets the CLI.target field based on the
	 ...
	 *
	 * @param args     Array of arguments passed in to the program's Main function.
	...
	public static void parse(String args[], String optnames[]) {
		...
		// Make sure that outfile is defined for assembly code-gen
		if (outfile == null && target == Action.ASSEMBLY) {
			// ext = .out
			outfile = infile + ext;
		}
	}
